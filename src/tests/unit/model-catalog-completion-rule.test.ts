...
describe('completion-model catalog rule', () => {
  ...
  it('withholds nothing new when the payload carries no ModelDetails scalars', async () => {
    ...
    // Without the ModelDetails scalars there is nothing to classify by, and the override table is
    // empty now that every id it held was observed carrying the markers live. An older provider
    // version that omits the scalars therefore publishes everything it advertises — the honest
    // outcome, since nothing in that payload says otherwise.
    expect(published).toEqual([
      'chat_20706',
      'gemini-3-flash',
      'gemini-3-pro',
