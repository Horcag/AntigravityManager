...
} from '@/modules/proxy-gateway/server/modules/shared/services/model-route-miss-journal.service';
...
describe('ModelRouteMissJournalService durability', () => {
  ...
  beforeEach(() => {
    ...
    filePath = path.join(directory, 'model-route-miss-journal.json');
  });
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        entries: [
          ...
          { key: 'broken', updatedAt: Date.now(), value: { model: 'broken', count: 'many' } },
        ],
      }),
      'utf-8',
    );
