...
- **Audio translation**: `POST /v1/audio/translations` is **not a vendor translation model**. The proxy first transcribes the audio, then submits that text for an English translation through the same configured model. It therefore takes two model passes instead of one, so its cost and latency differ from the vendor endpoint. Timestamp-bearing response formats are rejected: after text translation, their timings no longer line up with the audio. The endpoint only returns `json` or `text` and does not stream because the second pass cannot begin until the transcription completes.
...
- The stream keeps `reasoning_content` as a deliberate compatibility field. It is emitted only when upstream flags thinking (`thought`) or wraps content in `<think>...</think>`, so clients that currently read the extension can still render chain-of-thought without changing canonical `content`.
...
- This is an explicit boundary: `reasoning_content` is non-standard, so clients that do not rely on it should ignore it and use `content` as canonical output.
...
- `response_format: {"type":"json_object"}` guarantees parseability only, never shape. Upstream has two states and nothing between them: with a declared `responseSchema` the answer is clean but constrained to that shape, and a permissive schema discards the content instead of permitting everything; without one, `responseMimeType: "application/json"` still answers inside a markdown fence. OpenAI's "valid JSON of any shape" has no upstream equivalent, so the proxy removes the fence on the way out. It does so only when the request asked for `json_object`, the whole assistant content is exactly one fenced block tagged `json` or untagged with nothing outside it, and the block body parses as JSON; if any of the three fails, the content is returned byte-identical. A caller that needs the shape guaranteed should use `response_format: {"type":"json_schema"}`, which this transport genuinely enforces.
- The `json_object` unwrap covers every surface: the non-stream body, Chat Completions SSE — including the legacy `/v1/completions` wire shape and the synthesized stream used after a stream-path fallback — and the `/v1/responses` event stream, which is requested as `text.format: {"type":"json_object"}`. On any stream it withholds content only while the text so far could still be a fence opener; once the opener is confirmed the body is buffered to the end of the message, because "one block, nothing outside it, body parses" is undecidable until the content is complete. On `/v1/responses` that changes how many events carry the answer, so every event is renumbered: `sequence_number` stays monotonic and gapless, the event types and their order are unchanged, and `response.output_text.done`, `response.content_part.done`, `response.output_item.done` and the terminal `response.completed` payload all report the same text the deltas assembled. A request that did not ask for `json_object` streams byte-identically.
...
- Native Claude thought responses currently split the thought from a later answer part carrying `thoughtSignature`; that signature belongs to the answer part rather than the preceding thought. The emitted Anthropic `thinking` block is therefore unsigned, so a strict client cannot safely return that extended-thinking block on the next turn.
