...
- The `json_object` unwrap covers the non-stream body and Chat Completions SSE, including the legacy `/v1/completions` wire shape and the synthesized stream used after a stream-path fallback. On SSE it withholds content only while the text so far could still be a fence opener; once the opener is confirmed the body is buffered to the finish chunk, because "one block, nothing outside it, body parses" is undecidable until the content is complete. The `/v1/responses` event protocol is not rewritten, so `text.format: {"type":"json_object"}` still streams the fence there while its non-stream body is unwrapped.
