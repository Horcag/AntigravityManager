 ...
 * endpoint exists so a client still speaking the old dialect is not left with a
 * bare 404; it is deprecated at Anthropic too.
 */
...
export interface AnthropicMessagesShapedRequest {
  model: string;
  max_tokens: number;
 ...
 *
 * A prompt that never uses the `\n\nHuman:` marker is treated as a single user
 * turn rather than rejected, because that is what the old API accepted in
 * practice. The trailing `\n\nAssistant:` is the request for output and carries
 * no content unless the caller prefilled it, in which case the prefill is kept
 * as an assistant turn — that is exactly what it meant on the old endpoint.
 */
export function toAnthropicMessagesRequest(
  request: AnthropicCompleteRequest,
): AnthropicMessagesShapedRequest {
...
export function splitCompletionPrompt(
  ...
  // A trailing empty `Assistant:` is the ask for output, not a turn to send.
  return messages;
}
...
export function toAnthropicCompletionResponse(
  ...
  return {
    ...
    stop: typeof record.stop_sequence === 'string' ? record.stop_sequence : null,
 ...
 *
 * The request contracts carry their HTTP status as `status` while the file
 * errors carry it as `httpStatus`; this republishes the former under the name
 * the shared builder reads, so a validation failure is a 400 rather than the
 * 500 an unrecognised shape would produce.
 */
export function anthropicCompleteErrorResponse(
  ...
  const normalized =
    httpStatus === undefined && typeof status === 'number' && status >= 400 && status <= 599
      ? Object.assign(new Error(error instanceof Error ? error.message : 'Request failed'), {
          httpStatus: status,
        })
      : error;
  return anthropicFileErrorResponse(normalized, requestId);
}
