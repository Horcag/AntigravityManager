...
import { OpenAIResponsesSessionService } from '../openai/responses/openai-responses-session.service';
...
export class BatchRunnerService {
  ...
  constructor(
    ...
    @Optional() @Inject(ProxyService) private readonly proxyService?: ProxyService,
  ...
  private async runOne(job: BatchJobRecord, request: BatchRequestRecord): Promise<void> {
    const deps = this.executionDeps();
    const result = deps
      ? await executeBatchRequest(job, request, deps)
      : ({
          outcome: 'errored' as const,
          error: {
            message: 'The proxy service is not available to run batch requests',
            code: 'api_error',
            httpStatus: 503,
          },
        } as const);
    ...
    recordRequestOutcome(current, target, result, now);
  ...
  private executionDeps(): BatchExecutionDeps | null {
    if (!this.proxyService) {
      return null;
    }
    return {
      target: this.proxyService as unknown as BatchExecutionTarget,
      ...(this.fileStore ? { fileStore: this.fileStore } : {}),
      ...(this.responsesSessions ? { responsesSessions: this.responsesSessions } : {}),
    };
  }
