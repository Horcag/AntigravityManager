 ...
 *
 * Only the fields a poller actually needs are populated — `name`, `metadata`,
 * `done` and exactly one of `response` / `error`. Nothing beyond that is
 * fabricated, because a local runner has no equivalent for most of Google's
 * batch bookkeeping.
 */
...
export function toGeminiOperation(job: BatchJobRecord): GeminiOperationResource {
  ...
  const operation: GeminiOperationResource = {
    ...
    metadata: {
      ...
      model: job.endpoint.replace(/:batchGenerateContent$/u, ''),
