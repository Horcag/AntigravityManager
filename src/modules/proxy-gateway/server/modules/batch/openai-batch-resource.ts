 ...
 *
 * `/v1/embeddings` is refused by name because there is no embedding RPC on this
 * transport at all — established from the vendor's protobuf descriptors and
 * corroborated by `gemini-cli` implementing `embedContent()` as a `throw`. It
 * cannot be served in a batch because it cannot be served at all.
 */
export function requireServableEndpoint(endpoint: unknown): string {
  ...
  if (!(SERVABLE_BATCH_ENDPOINTS as readonly string[]).includes(normalized)) {
    throw BatchJobError.unservableEndpoint(normalized, SERVABLE_BATCH_ENDPOINTS);
  }
  return normalized;
}
